    ...
    for s in slicing:
        ...
        if isinstance(s, int):
            ...
            if s.ndim != 1:
                raise IndexError('Fancy slicing in dimension %d not supported!' % d)
            if len(s) != shape[d]:
                raise IndexError('The boolean indexing has different shape %d than the source %d in dimension %d!' % (
                len(s), shape[d], d))
            bool_array += 1
            if bool_array > max_arrays:
                raise IndexError('Boolean array slicing in dimension %d not supported!' % d)
    ...
    for s1 in slicing1:
        ...
        if isinstance(s1, int):
            ...
            if isinstance(s2, int):
                ...
                if len(s2) != shape2[d2]:
                    raise IndexError(
                        'Second boolean array slicing with shape %d is not of shape %d in dimension %d!' % (
                        len(s2), shape2[d2], d2))
            ...
            if len(s1) != shape1[d1]:
                raise IndexError('Boolean array slicing with shape %d is not of shape %d in dimension %d!' % (
                len(s1), shape1[d1], d1))
            s1 = np.where(s1)[0]
            ...
            if isinstance(s2, int):
                ...
                if len(s2) != shape2[d2]:
                    raise IndexError(
                        'Second boolean array slicing with shape %d is not an array with shape %d in dimension %d!' % (
                        len(s2), shape2[d2], d2))
                s1 = np.where(s1)[0]
                try:
                    slicing.append(s1[s2])
