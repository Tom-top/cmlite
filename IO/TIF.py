...
from utils.lazy import lazyattr
...
def create(location=None, shape=None, dtype=None, mode=None, as_source=True, **kwargs):
    ...
    memmap = tif.memmap(location, shape=shape, dtype=dtype, mode=mode)
    if as_source:
...
def data_size(filename, **args):
    ...
    return io.data_size_from_data_range(dims, **args)


def data_z_size(filename, z=all, **args):
...
def read_data(filename, x=all, y=all, z=all, **args):
    ...
    dsize = data_size(filename)
    # print "dsize %s" % str(dsize)

    if len(dsize) == 2:
        data = tif.imread(filename, key=0)
        # print "data.shape %s" % str(data.shape)

        return io.dataToRange(data.transpose([1, 0]), x=x, y=y)
        # return io.dataToRange(data, x = x, y = y)data_z_size
        ...
        if z is all:
            ...
            t = tif.TiffFile(filename)
            p = t.pages[0]
            data = np.zeros(ds, dtype=p.dtype)
...
def write_data(filename, data):
    ...
    if d == 2:
        tif.imwrite(filename, data.transpose([1, 0]))
    elif d == 3:
        tif.imwrite(filename, data.transpose([2, 1, 0]))
    elif d == 4:
        tif.imwrite(filename, data.transpose([2, 1, 0, 3]), photometric='minisblack', planarconfig='contig',
                    bigtiff=True)
    else:
