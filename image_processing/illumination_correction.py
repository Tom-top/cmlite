...
default_flat_field_line_file_name = os.path.join(settings.resources_path,
                                                 "Microscope/lightsheet_flatfield_correction.csv")
...
def correct_illumination(source, flatfield=None, background=None, scaling=None, dtype=None, verbose=False):
    ...
    if flatfield.shape[:2] != source.shape[:2]:
        raise ValueError(
            "The flatfield shape %r does not match the source shape %r!" % (flatfield.shape[:2], source.shape[:2]))
    flatfield = io.as_source(flatfield)
    ...
    if background is None:
        ...
        if background.shape != flatfield.shape:
            raise RuntimeError("Illumination correction: background does not match source shape: %r vs %r!" % (
            background.shape, source[:, :, 0].shape))
...
def flatfield_line_from_regression(source, sink=None, positions=None, method='polynomial', reverse=None,
                                   return_function=False, verbose=False):
