...
def filter_kernel_3d(ftype='Gaussian', shape=(5, 5, 5), sigma=None, sigma2=None, radius=None):
    ...
    if ftype == 'mean':  # differnce of gaussians
        ...
        ker = np.exp(-(x * x / 2. / (sigma[0] * sigma[0]) + y * y / 2. / (sigma[1] * sigma[1]) + z * z / 2. / (
                    sigma[2] * sigma[2])))
        return ker / ker.sum()
        ...
        ker = 1 - (x * x / 2. / (radius[0] * radius[0]) + y * y / 2. / (radius[1] * radius[1]) + z * z / 2. / (
                    radius[2] * radius[2]))
        ker[ker < 0] = 0.
        return ker / ker.sum()
        ...
        ker = np.exp(-(x * x / 2. / (radius[0] * radius[0]) + y * y / 2. / (radius[1] * radius[1]) + z * z / 2. / (
                    radius[2] * radius[2])))
        ker /= ker.sum()
        arg = x * x / math.pow(sigma[0], 4) + y * y / math.pow(sigma[1], 4) + z * z / math.pow(sigma[2], 4) - (
                    1 / (sigma[0] * sigma[0]) + 1 / (sigma[1] * sigma[1]) + 1 / (sigma[2] * sigma[2]))
        ker = ker * arg
        return ker - ker.sum() / len(ker)
        ...
        ker = np.exp(-(x * x / 2. / (sigma[0] * sigma[0]) + y * y / 2. / (sigma[1] * sigma[1]) + z * z / 2. / (
                    sigma[2] * sigma[2])))
        ker /= ker.sum()
        sub = np.exp(-(x * x / 2. / (sigma2[0] * sigma2[0]) + y * y / 2. / (sigma2[1] * sigma2[1]) + z * z / 2. / (
                    sigma2[2] * sigma2[2])))
        return ker - sub / sub.sum()
