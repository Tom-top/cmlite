...
#@cython.boundscheck(False)
#@cython.wraparound(False)
cpdef void devolve_weights_max(point_t[:,:] points, weight_t[:] weights, index_t[:,:] indices, sink_t[:] sink, index_t[:] shape, index_t[:] strides, int processes) nogil:
    """Converts a list of points into a volumetric image array, taking the maximum signal locally using weights."""
            ...
            for i in range(n_indices):
                ...
                for d in range(n_dim):
                    ...
                    if not (0 <= k and k < shape[d]):
                        ...
                        j = j + k * strides[d]
                if v == 1:
                    if sink[j] == 0:  # If the sink value is still the initial zero, set it to the weight
                        sink[j] = <sink_t>weights[n]
                    else:
                        sink[j] = max(sink[j], <sink_t>weights[n])  # Take the maximum value
