...
data_heatmaps_directory = os.path.join(heatmaps_directory, "MERFISH-C57BL6J-638850")
...
metadata_ccf = manifest['file_listing']['MERFISH-C57BL6J-638850-CCF']['metadata']
expression_matrices = manifest['file_listing']['MERFISH-C57BL6J-638850']['expression_matrices']

cell_metadata_path = expression_matrices['C57BL6J-638850']['log2']['files']['h5ad']['relative_path']
...
for n, g in enumerate(gene_names):
    ...
    if not os.path.exists(os.path.join(saving_directory, f"{g}_heatmap.nii.gz")):
        ...
        print(f"Gene {g} ({n}/{n_genes}). Original n cells: {n_cells_ccf}; Filtered n cells: {n_cells_filt}")
