...
DATASET_COLORS = ["#009E73", "#CC79A7", "#0072B2", "#E69F00", "#333333"]
...
MASK_DIR = os.path.join(MAP_DIR, "/default/path")  # PERSONAL
WHOLE_REGION = True  # If true, the unprocessed mask will be used
PLOT_COUNTS_BY_CATEGORY = False  # If true plots the category plot

if WHOLE_REGION:
    TISSUE_MASK = tifffile.imread(os.path.join(MASK_DIR, r"hemisphere_mask.tif"))
else:
    TISSUE_MASK = tifffile.imread(os.path.join(MASK_DIR, r"smoothed_mask.tif"))

MAIN_RESULTS_DIR = os.path.join(MAP_DIR, "results")
RESULTS_DIR = ut.create_dir(os.path.join(MAIN_RESULTS_DIR, "cells_to_reference"))
...
st_plt.plot_cells(all_animals_data, REFERENCE, TISSUE_MASK, cell_colors="#00796B",
                  ...
                  sg=sg, saving_path=os.path.join(os.path.dirname(SAVING_DIR), f"neurons_{ori}_merge_monochrome.png"),
                  relevant_categories=[], show_outline=False, zoom=False,
                  plot_individual_categories=False, surface_projection=True, save_mask=False)

st_plt.plot_cells(all_animals_data, REFERENCE, TISSUE_MASK, cell_colors="#FF6F61",
                  cell_categories=None, non_neuronal_mask=~all_non_neurons, xlim=xlim,
                  ylim=ylim,
                  orix=orix, oriy=oriy, orip=orix, ori=ori, mask_axis=mask_axis, s=cell_size,
                  sg=sg, saving_path=os.path.join(os.path.dirname(SAVING_DIR), f"non_neurons_{ori}_merge_monochrome.png"),
                  relevant_categories=[], show_outline=False, zoom=False,
                  plot_individual_categories=False, surface_projection=True, save_mask=False)

st_plt.plot_cells(all_animals_data, REFERENCE, TISSUE_MASK, cell_colors="#333333",
                  cell_categories=None, non_neuronal_mask=None, xlim=xlim,
                  ylim=ylim,
                  orix=orix, oriy=oriy, orip=orix, ori=ori, mask_axis=mask_axis, s=cell_size,
                  sg=sg, saving_path=os.path.join(os.path.dirname(SAVING_DIR), f"all_{ori}_merge_monochrome.png"),
                  relevant_categories=[], show_outline=False, zoom=False,
                  plot_individual_categories=False, surface_projection=True, save_mask=False)
