...
import utils.hierarchical_dict as hdict

import visualization.color as col

from alignment.utils import create_label_table
...
class Annotation(object):
    ...
    def initialize(self, label_file=None, extra_label=None, annotation_file=None):
        ...
        # import atlas
        self.atlas = io.read(self.annotation_file).astype(int)
    ...
    def get_lateralised_volume_map(self, atlas_scale, hemispheres_file_path):
        hemispheres_atlas = io.read(hemispheres_file_path)
    ...
    def get_dict_parents_to_children(self, parents_ids=None, including_parents=False):
        ...
        for parent_id in parents_ids:
            map_children[parent_id] = self.children_df.set_index('id').loc[
                parent_id, 'all_children_structures_ids'].copy()
    ...
    def get_dict_children_to_parents(self, parents_ids=None, including_parents=False):
        map_children = self.get_dict_parents_to_children(parents_ids=parents_ids,
                                                         including_parents=including_parents).copy()
        map_parent = {}
...
def set_annotation_file(annotation_file, label_file=None, extra_label=None):
    if label_file is None:
        initialize(annotation_file=annotation_file, label_file=annotation.label_file,
                   extra_label=annotation.extra_label)
    else:
        initialize(annotation_file=annotation_file, label_file=label_file, extra_label=extra_label)
...
def label_points(points, annotation_file=None, invalid=0, key='order', level=None):  # FIXME: document level
    ...
    atlas = io.read(__get_module_annotation_file(annotation_file))
...
def annotation_to_distance_file(annotation_file_path):
    brain_mask = (io.read(annotation_file_path) > 0).astype(int)
